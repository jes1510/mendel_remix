    ...
    case 150: // M150
      {
        ...
        if(code_seen('R')) red = (byte)code_value();
        if(code_seen('U')) grn = (byte)code_value();
        if(code_seen('B')) blu = (byte)code_value();
...
void handle_status_leds_RS(void) {
  float HE_diff = 0;
  float bed_diff = 0;
  int hysteresis = 5;
  int tempVal;
  ...
  if(millis() > stat_update) {
    ...
    stat_update += 2000; // Update every 2s
    
    
    
    //for (int8_t cur_extruder = 0; cur_extruder < EXTRUDERS; ++cur_extruder) {
   //    HE_diff = max(abs(degTargetHotend(cur_extruder) - degHotend(cur_extruder)), HE_diff);
       
   // }
 //   max_temp = max(HE_diff, hysteresis);    

    
    #if defined(TEMP_BED_PIN) && TEMP_BED_PIN > -1
      bed_diff = abs(degTargetBed() - degBed());
    //  max_temp = max(tempVal, bed_diff);
    ...
    else if(degHotend(0) > (degTargetHotend(0) + hysteresis) ) {  // Cooling
     r = 0;
     g = 0;
     b = 0xff;
     state = COOLING;
